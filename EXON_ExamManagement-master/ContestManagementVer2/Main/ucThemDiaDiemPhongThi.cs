...
namespace ContestManagementVer2.Main
{
    public partial class ucThemDiaDiemPhongThi : UserControl
    {
        ...
        private bool CheckPhongThi()
        {
            ...
            if (txtTenPhongThi.Text == "")
            {
                MessageBox.Show("Tên phòng thi không được để trống");
                return false;
            }
            else if (txtTenPhongThi.Text[0] != 'P')
            {
                MessageBox.Show("Tên phòng thi phải bắt đầu bằng ký tự 'P'");
                return false;
            }
            return true;
        }
        ...
        private void btn_ThemTenMay_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult confirm = MessageBox.Show("Bạn có chắc muốn sinh tên máy thi cho phòng thi?\nTên máy thi sẽ sinh tự động theo mẫu: Pi.MAYxx",
                    "Thông báo",
                    MessageBoxButtons.YesNo,
                    MessageBoxIcon.Warning);
                if (confirm == DialogResult.Yes)
                {
                    if (db.ROOMDIAGRAMS.Where(p => p.RoomTestID == tg.RoomTestID).Count() >= tg.MaxSeatMount)
                        MessageBox.Show("Phòng thi đã được sinh tên máy");
                    else
                    {
                        for (int i = 1; i <= tg.MaxSeatMount; i++)
                        {
                            ROOMDIAGRAM rd = new ROOMDIAGRAM();
                            rd.ComputerCode = tg.RoomTestName + "MAY" + i.ToString("00");
                            rd.ComputerName = tg.RoomTestName + "MAY" + i.ToString("00");
                            rd.RoomTestID = tg.RoomTestID;
                            rd.Status = 1;
                            db.ROOMDIAGRAMS.Add(rd);
                        }
                        db.SaveChanges();
                        MessageBox.Show("Sinh tên máy thi thành công: " + tg.MaxSeatMount.ToString() + " máy");
                    }
                }
            }
        }
