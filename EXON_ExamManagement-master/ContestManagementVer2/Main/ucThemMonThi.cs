...
namespace ContestManagementVer2.Main
{
    public partial class ucThemMonThi : UserControl
    {
            ...
            if (btnThem.Text == "Lưu")
            {
                if (CheckMonThi())
                {
                    try
                    {
                        ...
                        db.SCHEDULES.Add(tg);
                        db.SaveChanges();
                        //copy cấu trúc môn thi
                        STRUCTURE newstruc = new STRUCTURE();
                        STRUCTURES_SUBJECT oldstruc = db.STRUCTURES_SUBJECT.Where(x => x.SubjectID == tg.SubjectID).FirstOrDefault();
                        if (oldstruc != null)
                        {
                            newstruc.CreatedDate = oldstruc.CreatedDate;
                            newstruc.Status = oldstruc.Status;
                            newstruc.CreatedStaffID = oldstruc.CreatedStaffID;
                            newstruc.ScheduleID = tg.ScheduleID;
                            db.STRUCTURES.Add(newstruc);
                            db.SaveChanges();
                            STRUCTURE_DETAILS struc_detail;
                            foreach (STRUCTURE_SUBJECT_DETAILS item in oldstruc.STRUCTURE_SUBJECT_DETAILS)
                            {
                                struc_detail = new STRUCTURE_DETAILS();
                                struc_detail.NumberQuestions = item.NumberQuestions;
                                struc_detail.Level = item.Level;
                                struc_detail.Score = item.Score;
                                struc_detail.Status = item.Status;
                                struc_detail.StructureID = newstruc.StructureID;
                                struc_detail.TopicID = item.TopicID;
                                db.STRUCTURE_DETAILS.Add(struc_detail);
                                db.SaveChanges();
                            }
        ...
        private void btnKhoiThi_Click(object sender, EventArgs e)
        {
            //FrmKhoiThi frm = new FrmKhoiThi(kithi.ContestID, kithi.ContestName);
            FrmKhoiThi_2 frm = new FrmKhoiThi_2(kithi.ContestID, kithi.ContestName);
            frm.Show();
        }
