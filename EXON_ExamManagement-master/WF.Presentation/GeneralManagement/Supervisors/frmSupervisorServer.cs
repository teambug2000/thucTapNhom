...
namespace GeneralManagement.Supervisors
{
    public partial class frmSupervisorServer : MetroForm
    {
        ...
        private bool CheckedContestant = true;
        ...
        private List<UserGet> lstUserGet = new List<UserGet>();

        private int StatusDivisionShift;

        private int supervisorID;
        private int shiftID = 0;
        private int roomTestID = 0;
        private int divisionShiftID = 0;
        private bool isStartServer = false;
        private bool CheckDivisionTest = false; // trạng thái phát đề hay chưa
        private bool isCheckStartTest = false; // trạng thái bắt đầu thi hay chưa
        ...
        private List<ROOMDIAGRAM> lstRoomdiagram = new List<ROOMDIAGRAM>();

        private ROOMTEST room = new ROOMTEST();
        private SHIFT shift = new SHIFT();

        private int ctID;
        private Hashtable hashStatusLogin = new Hashtable(); // lưu trạng thái login của thí sinh tương ứng với

        private List<ContestantInformation> lstCIConnected = new List<ContestantInformation>();
        private int countDivisionShift = 0;
        ...
        private void ArrangeContestant()
        {
            ...
            if (listContestantShiftID.Count <= listRoomDiaID.Count)
            {
                ...
                foreach (DictionaryEntry entry in hasData)
                {
                    ...
                    if (!BusALL.Instance.AddTestNumberIndex(tn))
                    {
                        ArrangeContestant();
                        return;
                    }
        ...
        private bool CheckUserIDForStartTest(int id)
        {
            foreach (var item in tabControl1.SelectedTab.Controls)
            {
                if (item is Panel)
        ...
        #region Hỗ trỡ tạo phiếu

        private static void GenTestNumberindex(List<int> lstContestantShiftID, List<int> lstCountContestant, out Hashtable htbTestData)
        {
            Hashtable htbReturnData = new Hashtable();
            Random rnd = new Random();
            foreach (int contestantShiftID in lstContestantShiftID.ToList())
            {
                int rndvalue = rnd.Next(lstCountContestant.Count);
                htbReturnData.Add(contestantShiftID, lstCountContestant[rndvalue]);
                lstCountContestant.RemoveAt(rndvalue);
            }
            htbTestData = htbReturnData;
        }

        #endregion Hỗ trỡ tạo phiếu

        #region hỗ trợ xếp chỗ

        private static void ArrangeForContestant(List<int> lstContestantShiftID, List<int> lstRoomDiaID, out Hashtable htbTestData)
        {
        ...
        #endregion hỗ trợ xếp chỗ

        #region hỗ trợ phát đề

        private static void GenerateTestForContestant(List<int> lstContestantShiftID, List<int> lstTestID, out Hashtable htbTestData)
        {
        ...
        #endregion Các hàm load combobox máy có thể chuyển đến, load loại vi phạm
        ...
        private void serverSocket_ClientDisconnected(object sender, object data)
        {
            ...
            if (index > -1)
            {
                ...
                #endregion kiểm tra xem mất kết nối có chủ đích hay không. nếu ko ta cập nhật lại trạng thái và thời gian mất kết nối
            }

            client.Dispose();

            #endregion mat ket noi, kiêm tra neu là mat chu dong thi nó gui len 3004 mình cạp nhat trạng thái
        }
        ...
        private void client_Receive(object sender, object data)
        {
            ...
            #endregion Change UserTransformation from client
            ...
            #endregion nếu là trạng thái đăng nhập hoặc đăng nhập lại
            ...
            else
            {
                ...
                #endregion với trạng thái ko phải đăng nhập
            }
        ...
        #endregion Nhan tin hieu tu client
        ...
        #endregion xu ly update Status cho thi sinh, cap nhap button
        ...
        private void MenuItemDivisionShift_Click_1(object sender, EventArgs e)
        {
            ...
            #endregion Khai báo UserTransformation của máy server để gửi trên lstTraceLog
            ...
            #endregion update trang thái get test cho contestant shift
        ...
        private void MenuItemStartTest_Click(object sender, EventArgs e)
        {
            ...
            #endregion khởi tạo tên máy server để gửi dữ liệu
        ...
        #endregion menucontext phat de, bat dau lam bai,chuyen ca thi du phong cho tung UC
        ...
        private void mbtnDivsionTest_Click(object sender, EventArgs e)
        {
            ...
            if (listSchedule.Count > 0)
            {
                foreach (SCHEDULE item in listSchedule)
                {
                    ...
                    if (item != null && item.SubjectID > 0)
                    {
                        ...
                        if (listContestantShiftID.Count <= listTestIDForSubject.Count)
                        {
                            ...
                            #endregion tiến hành phát đề cho từng môn
                        }
                    }
                }

                #endregion lấy danh sách Schedule để lấy được các môn thi có trong KÍP thi TẠI PHÒNG THI này
                ...
                if (count_error_add_contestant == 0) // nếu không có lỗi trong quá trình phát đề thì /....
                {
                    ...
                    #endregion gửi trạng thái đến thí sinh khi phát đề thành công
                }
                else
                //{
                //}
        ...
        #endregion Su kien quy trinh phat de, bat dau ...
        ...
        private void btnViolation_Click(object sender, EventArgs e)
        {
            ...
            if (txtContestantID.Text != null && cmbTOV.SelectedValue != null)
            {
                ...
                #endregion gửi thông bảo cảnh cáo tới thí sinh
            }
            else
            {
        ...
        #endregion Sự kiện btn đổi ca, cảnh cáo, chuyển vị trí

        #region Thong ke thi sinh hoan thanh, danh sach xep cho

        private void mncPrintArr_Click(object sender, EventArgs e)
        {
            frmReportARR frm = new frmReportARR(divisionShiftID);
            frm.ShowDialog();
        }

        private void mncPrintResult_Click(object sender, EventArgs e)
        {
            frmReport frm = new frmReport(divisionShiftID, supervisorID, shiftID);
            frm.ShowDialog();
        }

        #endregion Thong ke thi sinh hoan thanh, danh sach xep cho
        ...
        private void mncPrintQuestionSpeaking_Click(object sender, EventArgs e)
        {
            ReportRoomDiagrams.FrmAllSpQues frm = new ReportRoomDiagrams.FrmAllSpQues(divisionShiftID);
            frm.ShowDialog();
        }
    }
}