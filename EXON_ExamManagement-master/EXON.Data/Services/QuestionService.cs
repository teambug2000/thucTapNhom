...
namespace EXON.Data.Services
{
    public interface IQuestionService
    {
        ...
        IEnumerable<QUESTION> GetByTopic(int idTopic);

        IEnumerable<QUESTION> GetByTopicNotAccepted(int idTopic);

        IEnumerable<QUESTION> GetByType(int idType);
    ...
    public class QuestionService : IQuestionService
    {
        ...
        {
            return _QuestionRepository.GetMulti(x => x.TopicID == idTopic
                && x.Status != (int)QuestionStatus.Accepted && x.Status != (int)QuestionStatus.Delete);
        }

        public IEnumerable<QUESTION> GetByTopic(int idTopic)
        {
            return _QuestionRepository.GetMulti(x => x.TopicID == idTopic
                && x.Status == (int)QuestionStatus.Accepted);
        }

        public IEnumerable<QUESTION> GetByType(int idType)
        {
            return _QuestionRepository.GetMulti(x => x.QuestionTypeID == idType);
        }

        public IEnumerable<QUESTION> GetByTopicNotAccepted(int idTopic)
        {
            return _QuestionRepository.GetMulti(x => x.TopicID == idTopic && x.Status != (int)QuestionStatus.Delete);
        }
    }
}