...
namespace EXON.Data.Services
{
    ...
    public class ContestantService : IContestantService
    {
        ...
        public void Delete(int id)
        {
            ...
            FingerPrinterService fgsv = new FingerPrinterService();
            ContestantShiftService contestantshiftsv = new ContestantShiftService();
            ContestantSubjectService consubsv = new ContestantSubjectService();
            CONTESTANT rg = GetById(id);

            if (rg != null)
            {
                List<FINGERPRINT> listginger = fgsv.GetByContestantID(id).ToList();
                foreach (var fgitem in listginger)
                {
                    fgsv.Delete(fgitem.FingerprintID);
                }
                fgsv.Save();
                contestantshiftsv.DeleteByContestant(rg.ContestantID);
                contestantshiftsv.Save();
                List<CONTESTANTS_SUBJECTS> lstconsub = rg.CONTESTANTS_SUBJECTS.ToList() ;
                foreach (var consubitem in lstconsub)
                {
                    consubsv.Delete(consubitem.ContestantSubjectID);
                    
                }
                consubsv.Save();
                rg = GetById(id);
                _ContestantRepository.Delete(rg);
                Save();
            }
        }
