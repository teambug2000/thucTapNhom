...
namespace CreateDB
{
    // Code chuyển ngược dữ liệu về trung tâm cũ của A Hoàng K50
    // Hiện tại không còn sử dụng nữa, nhưng vẫn ko nên xóa đi
    public partial class frmLoading2 : Form
    {
        ...
        // code chuyển dữ liệu về của a Hoàng K50
        ...
        public int Roomdiagram() //2.1. bang  ROOMDIAGRAM
        {
            ...
            foreach (Quiz.ROOMDIAGRAM itemtest in lsttest)
            {
                ...
                {
                    MessageBox.Show("Chuyển dữ liệu máy thi" + itemtest.RoomDiagramID.ToString() + " thất bại\n" + ex.ToString());
                }
            }
            return 1;
        }
            ...
            foreach (Quiz.QUESTION itemtest in lsttest)
            {
                try
                {
                    if (mainquiz.QUESTIONS.Where(x => x.QuestionID == itemtest.QuestionID).Count() == 0)
                    {
                        ...
                        bg.Type = itemtest.Type;
                        bg.Audio = itemtest.Audio;
        ...
        public int TestNumber() //12. bang  TESTNUMBER
        {
            status = 81;
            if (lbTest.InvokeRequired)
            {
            ...
            mainquiz = new QuizMain.QuizMain();
            quiz = new Quiz.Quiz();
            List<Quiz.TESTNUMBER> lsttest = new List<Quiz.TESTNUMBER>();
            lsttest = quiz.TESTNUMBERs.ToList();
            int count = 0;
            foreach (Quiz.TESTNUMBER itemtest in lsttest)
            {
                if (mainquiz.TESTNUMBERs.Where(x => x.IDTestNumber == itemtest.IDTestNumber).Count() == 0)
                {
                    QuizMain.TESTNUMBER bg = new QuizMain.TESTNUMBER();
                    bg.IDTestNumber = itemtest.IDTestNumber;
                    bg.TestNumberIndex = itemtest.TestNumberIndex;
                    bg.ContestantShiftID = itemtest.ContestantShiftID;

                    try
                    {
                        mainquiz.TESTNUMBERs.Add(bg);
                        mainquiz.SaveChanges();
                        count++;
                        bw.ReportProgress(count * 100 / lsttest.Count);
                    }
                    ...
                    {
                        MessageBox.Show("Chuyển số phách" + itemtest.IDTestNumber.ToString() + " thất bại\n" + ex.ToString());
                    }
                }
        ...
        public int AnswerSheetWriteSpeak() //11. bang  ANSWERSHEET_WRITING_SPEAKING
        {
            status = 81;
            if (lbTest.InvokeRequired)
            {
            ...
            mainquiz = new QuizMain.QuizMain();
            quiz = new Quiz.Quiz();
            List<Quiz.ANSWERSHEET_SPEAKING> lsttest = new List<Quiz.ANSWERSHEET_SPEAKING>();
            lsttest = quiz.ANSWERSHEET_SPEAKING.ToList();
            int count = 0;
            foreach (Quiz.ANSWERSHEET_SPEAKING itemtest in lsttest)
            {
                if (mainquiz.ANSWERSHEET_SPEAKING.Where(x => x.AnswerSheetSpeakingID == itemtest.AnswerSheetSpeakingID).Count() == 0)
                {
                    QuizMain.ANSWERSHEET_SPEAKING bg = new QuizMain.ANSWERSHEET_SPEAKING();
                    bg.AnswerSheetSpeakingID = itemtest.AnswerSheetSpeakingID;
                    bg.AnswerSheetID = itemtest.AnswerSheetID;
                    bg.SpeakingScore = itemtest.SpeakingScore;
                    bg.SpeakingScore2 = itemtest.SpeakingScore2;
                    bg.Status = itemtest.Status;
                    try
                    {
                        mainquiz.ANSWERSHEET_SPEAKING.Add(bg);
                        mainquiz.SaveChanges();
                        count++;
                        bw.ReportProgress(count * 100 / lsttest.Count);
                    }
                    ...
                    {
                        MessageBox.Show("Chuyển bài noi thí sinh" + itemtest.AnswerSheetID.ToString() + " thất bại\n" + ex.ToString());
                    }
                }

            }
            List<Quiz.ANSWERSHEET_WRITING> lsttestwrite = new List<Quiz.ANSWERSHEET_WRITING>();
            lsttestwrite = quiz.ANSWERSHEET_WRITING.ToList();
            int count2 = 0;
            foreach (Quiz.ANSWERSHEET_WRITING itemtest in lsttestwrite)
            {
                if (mainquiz.ANSWERSHEET_WRITING.Where(x => x.AnswerSheetWritingID == itemtest.AnswerSheetWritingID).Count() == 0)
                {
                    QuizMain.ANSWERSHEET_WRITING bg = new QuizMain.ANSWERSHEET_WRITING();
                    bg.AnswerSheetWritingID = itemtest.AnswerSheetWritingID;
                    bg.AnswerSheetID = itemtest.AnswerSheetID;
                    bg.WritingScore = itemtest.WritingScore;
                    bg.WritingScore2 = itemtest.WritingScore2;
                    bg.Status = itemtest.Status;
                    try
                    {
                        mainquiz.ANSWERSHEET_WRITING.Add(bg);
                        mainquiz.SaveChanges();
                        count++;
                        bw.ReportProgress(count * 100 / lsttest.Count);
                    }
                    ...
                    {
                        MessageBox.Show("Chuyển bài viết thí sinh" + itemtest.AnswerSheetID.ToString() + " thất bại\n" + ex.ToString());
                    }
                }
        ...
        public int Teacher() //11. bang  ANSWERSHEET
        {
            status = 81;
            if (lbTest.InvokeRequired)
            {
            ...
            mainquiz = new QuizMain.QuizMain();
            quiz = new Quiz.Quiz();
            List<Quiz.SPEAKING_TEACHER> lsttest = new List<Quiz.SPEAKING_TEACHER>();
            lsttest = quiz.SPEAKING_TEACHER.ToList();
            int count = 0;
            foreach (Quiz.SPEAKING_TEACHER itemtest in lsttest)
            {
                if (mainquiz.SPEAKING_TEACHER.Where(x => x.ID == itemtest.ID.ToString()).Count() == 0)
                {
                    QuizMain.SPEAKING_TEACHER bg = new QuizMain.SPEAKING_TEACHER();
                    bg.AnswerSheetSpeakingID = itemtest.AnswerSheetSpeakingID;
                    bg.ID = itemtest.ID.ToString();
                    bg.TeacherID = itemtest.TeacherID;
                    bg.Status = itemtest.Status;
                    try
                    {
                        mainquiz.SPEAKING_TEACHER.Add(bg);
                        mainquiz.SaveChanges();
                        count++;
                        bw.ReportProgress(count * 100 / lsttest.Count);
                    }
                    ...
                    {
                        MessageBox.Show("Chuyển bài noi thí sinh" + itemtest.ID.ToString() + " thất bại\n" + ex.ToString());
                    }
                }

            }
            List<Quiz.WRITING_TEACHER> lsttestwrite = new List<Quiz.WRITING_TEACHER>();
            lsttestwrite = quiz.WRITING_TEACHER.ToList();
            int count2 = 0;
            foreach (Quiz.WRITING_TEACHER itemtest in lsttestwrite)
            {
                if (mainquiz.WRITING_TEACHER.Where(x => x.ID == itemtest.ID).Count() == 0)
                {
                    QuizMain.WRITING_TEACHER bg = new QuizMain.WRITING_TEACHER();
                    bg.AnswerSheetWritingID = itemtest.AnswerSheetWritingID;
                    bg.ID = itemtest.ID;
                    bg.TeacherID = itemtest.TeacherID;
                    bg.Status = itemtest.Status;
                    try
                    {
                        mainquiz.WRITING_TEACHER.Add(bg);
                        mainquiz.SaveChanges();
                        count++;
                        bw.ReportProgress(count * 100 / lsttest.Count);
                    }
                    ...
                    {
                        MessageBox.Show("Chuyển bài viết thí sinh" + itemtest.ID.ToString() + " thất bại\n" + ex.ToString());
                    }
                }
