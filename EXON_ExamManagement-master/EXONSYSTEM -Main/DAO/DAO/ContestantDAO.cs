                ...
                try
                {
                    ...
                    if (CS != null)
                    {
                        ...
                        if (CSH.ROOMDIAGRAM.ComputerName == ComputerName)
                        {
                            ...
                            if (CI.TimeRemained > 0)
                            {
                                CI.Status = CSH.Status;
                                //CI.ShiftID = CS.ShiftID;

                                if (CSH.Status == Common.STATUS_DOING_BUT_INTERRUPT || CSH.Status == Common.STATUS_DOING || CI.Status == Common.STATUS_LOGGED_DO_NOT_FINISH)
                                {
                            ...
                            {
                                EC = new ErrorController(Common.STATUS_lOGIN_OUTTIME, "Login OutTime");
                                rCI = null;
                            }
                        }
                        else
