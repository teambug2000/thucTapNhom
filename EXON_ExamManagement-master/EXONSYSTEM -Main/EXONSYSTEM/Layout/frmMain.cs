...
namespace EXONSYSTEM
{
    public partial class frmMainForm : Form
    {
                ...
                if (obj.Question.Count > 1)
                {
                    ...
                    else if (qHeader.FormatQuestion == Constant.FORMAT_QUESTION_RTF)
                    {
                        ...
                        else
                        {
                            ...
                            lbl.Width = flpnMultiQuestion.Width;
                            lbl.Font = new  Font(Constant.FONT_FAMILY_DEFAULT, Constant.FONT_SIZE_DEFAULT, FontStyle.Bold);
             
                            Panel pnlBtnAudio = new Panel();
                            pnlBtnAudio.Width = flpnMultiQuestion.Width;
                            // button play Audio
                            Button btnPlayAudio = new Button();
                            btnPlayAudio.Location = new Point(5,10);
                            btnPlayAudio.Height = Properties.Resources.PLAY_MEDIA.Height;
                            btnPlayAudio.Width = Properties.Resources.PLAY_MEDIA.Width;
                            btnPlayAudio.FlatStyle = FlatStyle.Flat;
                            btnPlayAudio.FlatAppearance.MouseOverBackColor = Color.White;
                            btnPlayAudio.Cursor = Cursors.Hand;
                            btnPlayAudio.FlatAppearance.BorderSize = 0;
                            btnPlayAudio.BackgroundImage = Properties.Resources.PLAY_MEDIA;
                            //button stop audio     
                            Button btnStopAudio = new Button();
                            btnStopAudio.Location = new Point(btnPlayAudio.Width + 8, 10);
                          
                            btnStopAudio.Width = 48;
                            btnStopAudio.Height = 48;
                            btnStopAudio.FlatStyle = FlatStyle.Flat;
                            btnStopAudio.FlatAppearance.MouseOverBackColor = Color.White;
                            btnStopAudio.Cursor = Cursors.Hand;
                            btnStopAudio.FlatAppearance.BorderSize = 0;
                            btnStopAudio.BackgroundImage = Properties.Resources.STOP_MEDIA;
                            /// track bar audio                                          
                            Label lblMaximumLenght = new Label();
                            Label lblSeek = new Label();
                            TrackBar mtbAudio = new TrackBar();
                            lblMaximumLenght.Font = new Font(Constant.FONT_FAMILY_DEFAULT, Constant.FONT_SIZE_DEFAULT, FontStyle.Bold);
                            lblSeek.Font = new Font(Constant.FONT_FAMILY_DEFAULT, Constant.FONT_SIZE_DEFAULT, FontStyle.Bold);
                            lblSeek.Location = new Point(btnPlayAudio.Width+btnStopAudio.Width+8, 10);

                            lblSeek.Text = "00:00:00";
                          
                            mtbAudio.Location = new Point(lblSeek.Width+btnPlayAudio.Width + btnStopAudio.Width , 10);
                            mtbAudio.Width = 350;
                            lblMaximumLenght.Location = new Point(btnPlayAudio.Width + lblSeek.Width + mtbAudio.Width + btnStopAudio.Width+5, 10);
                            lblMaximumLenght.Text = "00:00:00";

                            btnPlayAudio.Click += (sender, e) => PlayMusicEvent(sender, e,buffer,sound, mtbAudio,lblSeek,lblMaximumLenght,btnStopAudio);
                            ///control add
                            pnlBtnAudio.Controls.Add(btnPlayAudio);
                            pnlBtnAudio.Controls.Add(btnStopAudio);
                            pnlBtnAudio.Controls.Add(lblSeek);
                            pnlBtnAudio.Controls.Add(mtbAudio);
                            pnlBtnAudio.Controls.Add(lblMaximumLenght);
                            ...
                            rtbTitleOfQuestion.Rtf = qHeader.TitleOfQuestion;
                            rtbTitleOfQuestion.Width = flpnMultiQuestion.Width - 30;
                            rtbTitleOfQuestion.Height += 10;
                            rtbTitleOfQuestion.Margin = new Padding(15, 5, 0, 5);
                            rtbTitleOfQuestion.ReadOnly = true;
                            obj.Question.RemoveAt(0);
                            flpnMultiQuestion.Controls.Add(lbl);
                            flpnMultiQuestion.Controls.Add(pnlBtnAudio);
                            flpnMultiQuestion.Controls.Add(rtbTitleOfQuestion);
                        }
        ...
        /// <param name="mtbAudio"></param>
        /// <param name="lblSeek"></param>
        /// <param name="lblMaximum"></param>
        private void PlayMusicEvent(object sender, EventArgs e, byte[] buffer, string filename, TrackBar mtbAudio, Label lblSeek, Label lblMaximum, Button btnStopAudio)
        {

            Button btn = (Button)sender;
           btn.BackgroundImage = Properties.Resources.PLAY_MEDIA;            
            ISoundSource source = engine.AddSoundSourceFromMemory(buffer, filename);
         btn.Enabled = false; 
            Type a = engine.GetType();
            currentlyPlayingSound= engine.Play2D(filename);
            btnStopAudio.Enabled = true;
            btnStopAudio.Click += ( send, ev) => StopMusicEvent(currentlyPlayingSound);
            int convertedTime = (int)currentlyPlayingSound.PlayLength / 1000;
            TimeSpan soundTime = TimeSpan.FromSeconds(convertedTime);
            lblMaximum.Text = soundTime.ToString();
            mtbAudio.Maximum = (int)currentlyPlayingSound.PlayLength;

            mtbAudio.Minimum = 0;
            System.Windows.Forms.Timer time = new System.Windows.Forms.Timer();
            time.Start();

            time.Tick +=(sendertime,ev)=>  Time_Tick(mtbAudio,lblSeek);
          
        }

        private void StopMusicEvent(ISound currentlyPlayingSound)
        {
            currentlyPlayingSound.Stop();
        }

        private void Time_Tick(TrackBar mtbAudio,Label lblSeek)
        {
            mtbAudio.Value = (int)currentlyPlayingSound.PlayPosition;
            timeConverter(lblSeek);
            mtbAudio.Update();
        }

        private void timeConverter(Label lblseek)
        {
            int convertedTime = (int)currentlyPlayingSound.PlayPosition / 1000;


            TimeSpan soundTime = TimeSpan.FromSeconds(convertedTime);

            lblseek.Text = soundTime.ToString();
        }
            ...
            foreach (Control e in lstControlQuestions)
            {
                if (e.GetType().ToString().EndsWith(Constant.STRING_FLOWLAYOUTPANEL))
                {
                    foreach (Control c in e.Controls)
                    {
                        ...
                        {
                            if ((c as ucQuestionsRTF).q.Type == (int)Constant.QuizTypeEnum.Fill || (c as ucQuestionsRTF).q.Type == (int)Constant.QuizTypeEnum.FillAudio || (c as ucQuestionsRTF).q.Type== (int)Constant.QuizTypeEnum.Essay)
                            {
                                AnswersheetDetail ansd = new AnswersheetDetail();
                                ansd = (c as ucQuestionsRTF).AD;
                                if(ansd.AnswerContent!=null)
                                    AnswersheetDetailBUS.Instance.PushAnswerSheetDetail(ansd,out rEC);
                                if (rEC.ErrorCode == Constant.STATUS_OK)
                                {        
