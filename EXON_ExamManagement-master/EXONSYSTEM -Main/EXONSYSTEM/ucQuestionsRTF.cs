    ...
    public partial class ucQuestionsRTF : MetroUserControl
    {
        ...
        private void ucQuestionsRTF_Load(object sender, EventArgs e)
        {
            ...
            if (q.Type == 2 || q.Type == 3)
            {
                ...
                lblAnswser.Text = "Answer:";
                lblAnswser.Location = new Point(locationLeftRadio, 5);
                mpnAnswers.Controls.Add(lblAnswser);
                              
                rtbAnswerA.Location = new Point(lblAnswser.Right + 8, 3);
                rtbAnswerA.Width = 600;
                rtbAnswerA.TextChanged += RtbAnswerA_TextChanged;
                ...
                rtbAnswerA.ReadOnly = false;
                rtbAnswerB.Visible = false;
                rtbAnswerC.Visible = false;
                rtbAnswerD.Visible = false;
                mpnAnswers.Height = Controllers.Instance.GetHeightBetter(rtbAnswerA.Bottom, lblAnswser.Bottom)+10;
            }
            else if(q.Type== 4)
            {
                ...
                rtbAnswerA.Location = new Point(lblAnswser.Right + 8, 3);
                rtbAnswerA.Width = 600;
                rtbAnswerA.Height = 400;
                rtbAnswerA.Font = new Font(Constant.FONT_FAMILY_DEFAULT, Constant.FONT_SIZE_DEFAULT, FontStyle.Regular);
                rtbAnswerA.TextChanged += RtbAnswerA_TextChanged;
                Binding dbTitleOfQuestion = new Binding("Rtf", q, "TitleOfQuestion");
                rtbTitleOfQuestion.DataBindings.Add(dbTitleOfQuestion);
                mrbAnswerA.Visible = false;
            ...
            else
            {
                ...
                mpnAnswers.Height = Controllers.Instance.GetHeightBetter(rtbAnswerD.Bottom, mrbAnswerD.Bottom);
                 ...
                 else if(q.ListAnswer.Count==3)
                {
                    rtbAnswerD.Visible = false;
                    mrbAnswerD.Visible = false;
                    mpnAnswers.Height = Controllers.Instance.GetHeightBetter(rtbAnswerC.Bottom, mrbAnswerC.Bottom);
                }
                else
                {
                    rtbAnswerD.Visible = false;
        ...
        private void RtbAnswerA_TextChanged(object sender, EventArgs e)
        {
          ...
          AD.LastTime = Controllers.Instance.ConvertDateTimeToUnix(DAO.DAO.ConvertDateTime.GetDateTimeServer());
            AD.AnswerContent = rtbAnswerA.Text;
