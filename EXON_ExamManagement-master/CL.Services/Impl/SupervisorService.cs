...
namespace CL.Services.Impl
{
    public partial class SupervisorService : ISupervisorService
    {
        ...
        #endregion Staff And supervisor
        ...
        public int CheckLogin(string user, string pass, out int per)
        {
            /// chay thu
            per = -1;
            Db = new MTAQuizEntities();
            {
                try
                {
                    ...
                    var staffList = (from obj in Db.EXAMINATIONCOUNCIL_STAFFS
                                     ...
                                     where ((obj.StaffID == obj3.SupervisorID) && (obj.UserName == user && (obj.Password == pass)))
                                     select obj).ToList();
                    if (staffList == null || staffList.Count == 0)
                        return 0;
                    else
                        return (int)staffList.First().StaffID;
                }
                catch (Exception ex)
                {
                    string a = ex.Message;
        ...
        #endregion DivisionShift

        public SHIFT GetShiftByDivsionShiftID(int divisionShiftID)
        {
            SHIFT result = new SHIFT();
            DIVISION_SHIFTS ds = new DIVISION_SHIFTS();
            Db = new MTAQuizEntities();
            try
            {
                ds = Db.DIVISION_SHIFTS.Where(x => x.DivisionShiftID == divisionShiftID).SingleOrDefault();
                if (ds != null)
                {
                    result = Db.SHIFTS.Where(x => x.ShiftID == ds.ShiftID).SingleOrDefault();
                    return result;
                }
                else
                {
        ...
        //
        public bool AddTestNumberIndex(TESTNUMBER _tn)
        {
            using (MTAQuizEntities db = new MTAQuizEntities())
            {
                try
